...
This document defines the core Components and the standard **archetypes** — characteristic combinations of Components that define categories of Entities such as Models, Units, Items, Commanders, Armies, and Battleforces. Every rule in other sections (Battle Rules, Combat Resolution, Movement, etc.) operates on the structures defined here.

This document builds bottom-up: foundational concepts first (Entities, Components, marker Components), then the data Components that describe capability (Statline, Items, Attack Profiles), then the archetypes that combine them (Models, Units, Abilities), and finally the organizational archetypes that group them (Commanders, Armies, Battleforces).
...
- **Entity Tags** identify what is inherent to an Entity, such as an Infantry Model having the `Infantry` marker Component.
...
Rules that reference a marker Component always specify the exact name. A rule that says "Units with the Infantry tag" affects only Entities that have the `Infantry` marker Component.

### 200.4 Systems

A System is a unit of logic that operates on Entities possessing a specific set of Components. Systems read Component data, perform calculations, and write results back to Components. Systems contain all game logic; Components contain all game state.
...
|---|---|
| Model | Statline, Equipped Items, Destruction State |
| Unit | Statline, Faction Keywords, Unit Keywords, Composition, Abilities, Value, Zone, Morale State |
...
| Army | Commander Reference, Unit References, Army marker Component |
| Battleforce | Army Reference, Unit References, Total Value, Battleforce marker Component |
...
- **Own**: Objects that belong to the reference Entity. A Commander's Own Units are the Units in that Commander's Army.
...
The Statline Component is a data Component holding the base Characteristics shared by Models. Every Entity of the Model archetype has a Statline Component with exactly five Characteristics: Endurance, Durability, Morale, Speed, and Reflex. Each Characteristic is an integer value greater than or equal to 0.

Models in the same Unit may have different Statline Component values. The Combat System operates at the Model level, using each individual Model's own Statline Component values (see Combat Resolution in Terminology).

A Model's Statline Component can be modified by equipped Item Components, Ability Components, and rules affecting the Model or the Model's Unit. Modifications are applied as additive or multiplicative adjustments and are tracked separately from the base value.

### 210.2 Endurance

Endurance is an integer Characteristic in the Statline Component representing a Model's capacity for sustained exertion. Actions that tax Endurance (such as the Run or Charge Orders; see rules 132.2d and 132.2e in Battle Rules) reduce the Unit's Endurance by the specified amount. When a Unit's remaining Endurance reaches 0, that Unit may not receive Orders that tax Endurance.

Endurance recovers when a Unit performs actions that do not tax it. The default recovery rate is 1 Endurance per Turn for Units that did not perform an Endurance-taxing action during the previous Turn (see rule 131.1 in Battle Rules).

Baseline Competence — a Model's general effectiveness when making attacks — is derived from Endurance by the Combat System (see rule 210.7a).

### 210.3 Durability

Durability is an integer Characteristic in the Statline Component representing how much damage a Model can absorb before being destroyed. When the Combat System resolves a Damage Instance whose mitigated value meets or exceeds the Model's Durability, that Model is destroyed.

A single Damage Instance can destroy at most one Model. Damage that exceeds the target Model's Durability does not carry over to other Models. Sub-lethal damage (damage below Durability) is tracked on the Model's Damage Taken Component for future resolution.
...
Every Unit has a **Morale Baseline** value in its Morale State Component, which is the Morale state the Morale System trends the Unit toward over time. The default Morale Baseline is Neutral.

The Morale System tests casualties against the Morale Characteristic: if casualties exceed the Morale Characteristic, the Unit's Morale state worsens by one stage; if casualties are fewer than or equal to the Morale Characteristic, the Morale state improves by one stage (maximum Good). Surging is not reached through Morale Tests; it is reached only when a condition in one of the Unit's Ability Components explicitly sets Morale to Surging. See rule 134.1 in Battle Rules.

During the Rally Phase, the Morale System moves each Unit's Morale state one stage toward the Unit's Morale Baseline. If the Unit's Morale state is Surging, it resets to Neutral instead (see rule 131.1 in Battle Rules).

Morale state affects which Orders and actions a Unit may take:
- A Unit with Broken Morale may not receive the Charge or Surge Action Orders (see rule 132.4 in Battle Rules).
- A Unit with Surging Morale may perform a Surge Action (see rule 132.2h in Battle Rules).

### 210.5 Speed

Speed is an integer Characteristic in the Statline Component representing the typical distance a Unit can move in a single Turn without taxing Endurance. The Movement System reads Speed from the Statline Component.

- **Advance**: move up to Speed distance units (rule 132.2b in Battle Rules).
- **Run**: move up to 2 × Speed distance units; taxes Endurance (rule 132.2d in Battle Rules).
- **Charge**: move up to floor(1.5 × Speed) distance units toward a target; taxes Endurance (rule 132.2e in Battle Rules).
- **Fall Back**: move up to Speed distance units away from the nearest enemy Unit; a Unit with Broken Morale moves up to 2 × Speed instead (rule 132.2g in Battle Rules).

Speed also contributes to a Model's Evasion Rating (see rule 210.7b).

### 210.6 Reflex

Reflex is an integer Characteristic in the Statline Component representing a Model's ability to react to unexpected events. Reflex determines:

- Whether a Unit may declare a Reposition or Evade Reaction (Reflex must be greater than 0; see rules 133.3a and 133.3b in Battle Rules).
- The magnitude of an Evade Reaction's Evasion bonus (equal to the Unit's Reflex value; see rule 133.3b in Battle Rules).

Reflex contributes to a Model's Evasion Rating (see rule 210.7b).
...
Baseline Competence represents a Model's general effectiveness when making attacks. The Combat System derives Baseline Competence from the Model's Statline Component's Endurance value. Weapon Modifier Components and situational effects adjust the final hit probability relative to Baseline Competence.

#### 210.7b Evasion Rating

Evasion Rating represents how difficult a Model is to hit. The Combat System derives Evasion Rating from the Model's Statline Component's Reflex and Speed values, plus situational modifiers (such as the `Dug In` marker Component or the Evade Reaction).

#### 210.7c Armour Value

Armour Value is the sum of the Armour Value properties from all Armour Components on Items equipped to the Model. The Combat System uses Armour Value to reduce incoming damage after a hit is confirmed. The interaction between Armour Value, Armour Resistances, and Damage Types is specified in the Combat Resolution rules.

### Examples

#### Example 1: Reading a Statline Component
**Setup**: A Rifleman Model's Statline Component contains Endurance 10, Durability 10, Morale 10, Speed 10, Reflex 10.
**Meaning**: The Model can sustain moderate exertion, absorb 10 damage before the Combat System marks it as destroyed, has average morale resilience, the Movement System moves it 10 distance units per Advance, and it has decent reactive capability.

#### Example 2: Asymmetric Statline Component
**Setup**: A Maw Fiend Model's Statline Component contains Endurance 15, Durability 100, Morale 20, Speed 8, Reflex 12.
...
An Item is an Entity of the Item archetype. An Item's Components provide additional rules, Characteristics, or capabilities to the Model that equips it. An Item Entity has:
...
An Item may have marker Components (Tags). An Item that is equipped to a Model does not automatically confer its marker Components to the Model or the Model's Unit. If an Item confers marker Components to its bearer or bearer's Unit, the Item's rules state this explicitly.
...
A Weapon is an Item with Weapon in its Item Type Component. A Weapon provides one or more Attack Profile Components to the Model that equips it. Each Attack Profile Component describes a single way the Weapon can be used.
...
| Bracket | Description |
|---|---|
| **Min** | Minimum targeting distance. Attacks cannot target Models within Min distance. |
| **Short** | Beginning of effective range. |
| **Long** | End of effective range. |
| **Max** | Maximum targeting distance. Attacks cannot target Models beyond Max distance. |
...
For melee Attack Profile Components, Range is limited to a Max distance (default 1 distance unit) representing close proximity. Melee profiles have no Min, Short, or Long brackets.

All Range values are integers representing distance units.
...
A Model may equip multiple Items with Armour Components. The Model's total Armour Value is the sum of all equipped Armour Components' Armour Values. Resistances and Hit Modifiers stack additively across equipped Armour Components.
...
A Consumable Component tracks a finite use count. Each use expends one charge. When all charges are expended, the Consumable is no longer available for use until restored by a rule. The number of charges is defined by the count of the Item in the Model's Equipped Items Component.
...
A Model is an Entity of the Model archetype — its Components include a Statline Component, an Equipped Items Component, and a Destruction State Component. Models are the most granular interactive Entities in Simple War. Each Model Entity has:

- **ID**: A unique string identifier.
- **Display Name Component**: A human-readable label.
- **Equipped Items Component**: A list of Item references, each with a count, defining the Model's equipment loadout.
...
- **Destruction State Component**: Boolean indicating whether the Model is destroyed.

Models define how a Unit's baseline capability declines as it takes casualties: each Model contributes to the Unit's ability to affect an engagement, and the Combat System removing a Model reduces that contribution.

### 230.2 Bearer Relationship

When a Model equips an Item, the Model is the Item's **Bearer**. The Bearer relationship establishes:

1. The Model gains access to all rules, Attack Profile Components, and Statline modifications provided by the Item's Components.
2. The Model does **not** automatically gain the Item's marker Components (Tags). If an Item confers marker Components to its Bearer or Bearer's Unit, the Item's rules state this explicitly.
3. A Model may equip multiple Items. Equipment categories (e.g., "a Model may equip at most one Item with a heavy_plate Armour Component") are defined by the Unit's Composition rules or by Item-specific restrictions.
4. When the Combat System marks a Model as destroyed, all Items in that Model's Equipped Items Component remain associated with the destroyed Model in the Casualty Report but are no longer in play.

### Examples

#### Example 1: Basic Model
**Setup**: A Rifleman Model Entity is equipped with a Human Rifle (count 1), Human Combat Armour (count 1), a Combat Knife (count 1), and Frag Grenades (count 3).
**Meaning**: The Combat System reads the Model's Attack Profile Components from the Rifle and Knife, Armour Component from the Combat Armour, and the Consumable Component from the Frag Grenades (3 charges).
...
A Unit is an Entity of the Unit archetype — its Components include a Statline Component, Faction Keyword marker Components, Unit Keyword marker Components, a Composition Component, Ability Components, a Value Component, and a Zone Component. Units group Models, providing the collective interface through which those Models engage with Zones, other Entities, and the rules system. Units are the primary Entities that Players interact with when issuing Orders (see rule 132.1 in Battle Rules), moving on the Battlefield, and tracking state.

A Unit Entity has:

- **ID**: A unique string identifier.
- **Display Name Component**: A human-readable label.
- **Faction Keyword marker Components**: One or more marker Components identifying the Unit's allegiance (e.g., `Human`, `Xenos`, `Chaos`).
- **Unit Keyword marker Components**: One or more marker Components describing the Unit's battlefield role (e.g., `Infantry`, `Vehicle`, `Monster`).
- **Statline Component**: The Characteristic values for Models in the Unit (see rule 210.1). Individual Models may have different values as defined by the Composition.
- **Value Component**: An integer representing the Unit's strength for armybuilding and scoring purposes (see rule 240.6).
- **Ability Components**: A list of Ability references (see rule 250.1).
- **Composition Component**: The Models and their counts that make up the Unit (see rule 240.2).
- **Model Options Component**: Optional additional or replacement Models that can be added during armybuilding.
- **Item Options Component**: Optional equipment changes that can be made to Models during armybuilding.
- **Zone Component**: The Unit's current Zone, managed by the Zone System (e.g., Battlefield, Reserves, Embarked, Casualty Report).
- **Morale State Component**: The Unit's current Morale state and Morale Baseline, managed by the Morale System (see rule 210.4).
- **Endurance Remaining Component**: Integer tracking current Endurance for Orders that tax it.

### 240.2 Composition Component

A Unit's Composition Component defines which Models make up the Unit and in what quantities. Each entry specifies:

- **Model ID**: A reference to a Model definition (see rule 230.1).
- **Count**: The number of that Model in the Unit (integer ≥ 1).

The total number of Models in a Unit is the sum of all Composition entry counts. A Unit must have at least one Model.

**Model Options** allow Players to add Models to or replace Models in the Unit during armybuilding. Each Model Option specifies the Model to add, the count, the value cost, and any conditions (e.g., "available only if Unit size is 20 or more").

**Item Options** allow Players to change equipment on specific Models during armybuilding. Each Item Option specifies the Item to add, the Model receiving it, the count, the value cost, and optionally the Item being replaced.

### 240.3 Tag Inheritance

A Unit inherits all marker Components (Tags) from all Models in the Unit. If any Model in the Unit has a marker Component, the Unit has that marker Component.

Tag inheritance is recalculated whenever the Unit's Model composition changes (e.g., the Combat System destroys a Model or a Model is added). If all Models with a particular marker Component are removed from the Unit, the Unit loses that marker Component.

### 240.4 State Inheritance

A Unit can inherit states from its Models when **all** Models in the Unit share that state. The primary example: the Zone System transitions a Unit to the Casualty Report only when every Model in the Unit is destroyed.

A Unit is **not** in a given state if at least one Model in the Unit is not in that state, unless a rule explicitly states otherwise.

### 240.5 Datasheets

A Datasheet is a comprehensive reference document for a Unit, compiled from the Unit's Component data for Player convenience. A Datasheet contains:

- Unit name, Faction Keyword marker Components, and Unit Keyword marker Components
- Composition Component (default Models, Model Options, Item Options with costs)
- Statline Component values (Characteristics for each Model type in the Unit)
- Default equipment (Items in each Model's Equipped Items Component by default)
- Ability Components
- Value Component
- Restrictions (if any)

A Datasheet does not contain rules that are not part of the Unit's Component data. A Model's Datasheet shows its baseline combat Characteristics as determined by its default Item Components before any options are chosen.

### 240.6 Value Component

The Value Component holds an integer representing the Unit's overall strength for armybuilding, Roster validation, and scoring. Each Unit definition specifies a base Value. Model Options and Item Options modify the Unit's Value by their specified costs.

The total Value of a Battleforce is the sum of all Unit Value Components (including selected options) in that Battleforce. Rosters and Campaigns may impose caps or bands on total Battleforce Value (see rule 270.2).

### Examples

#### Example 1: Unit Composition Component
**Setup**: A Riflemen Unit's Composition Component lists: 10 × Rifleman Model. Each Model's Statline Component has Endurance 10, Durability 10, Morale 10, Speed 10, Reflex 10. The Unit's Value Component is 100.
**Meaning**: The Unit consists of 10 identical Model Entities. The Unit's collective behavior is governed by Orders issued to the Unit, but the Combat System resolves damage per Model.

#### Example 2: Model Option
**Setup**: The Riflemen Unit has a Model Option: add 10 more Rifleman Models for +100 Value, not required.
**Meaning**: During armybuilding, the Player may choose to increase the Composition Component from 10 to 20 Models at an additional cost of 100 to the Value Component.

#### Example 3: Item Option with Replacement
**Setup**: The Riflemen Unit has an Item Option: replace one Human Rifle with a Rocket Launcher for +20 Value.
**Meaning**: During armybuilding, the Player may swap one Model's Equipped Items Component entry, changing that Model's available Attack Profile Components and increasing the Unit's Value Component by 20.

#### Example 4: State Inheritance
**Setup**: A Riflemen Unit has 10 Model Entities. The Combat System has destroyed 9; 1 survives.
**Result**: The Unit is not destroyed. The surviving Model continues to operate under the Unit's rules. If the Combat System destroys the last Model, the Zone System transitions the Unit to the Casualty Report.
...
An Ability Component is a named rule or set of rules attached to a Unit Entity that provides capabilities beyond the Unit's base Statline and equipment Components. The Ability System evaluates Ability Components during appropriate timing windows. Each Ability Component has:
...
A Unit has zero or more Ability Components of each type (Passive, Active, Surge). The Unit's definition specifies the exact Ability Components available. Additional Ability Components may be granted by equipped Item Components, Campaign rules, or other Entities.

### 250.2 Passive Ability Components

A Passive Ability Component is always in effect while the Unit is in play and meets any stated conditions. The Ability System applies Passive Ability Components continuously — they do not require activation, do not cost an action, and cannot be voluntarily suppressed by the owning Player.
...
A Surge Ability Component is a special Active Ability Component available only when the Unit's Morale State Component is Surging. Activating a Surge Ability Component requires the Surge Action Order (see rule 132.2h in Battle Rules). After the Ability System resolves the Surge Ability, the Morale System resets the Unit's Morale to Neutral during the next Rally Phase.

A Unit may perform at most one Surge Action per Battle unless a rule explicitly grants additional uses.

**Rule statement form**: `[Surge] When this Unit performs a Surge Action, [effect].`
...
**Type**: Passive
**Effect**: While this Unit is within 3 distance units of an Objective marker and is not destroyed, the Ability System counts this Unit as controlling that Objective, even if Enemy Units are also within 3 distance units — unless those Enemy Units also have the Objective Secured Ability Component.

#### Example 2: Passive Ability Component — Forward Positions
**Name**: Forward Positions
**Type**: Passive
**Effect**: During the Deployment Stage, this Unit may be placed up to 6 distance units beyond the owning Player's Deployment Zone boundary, provided the final position is more than 9 distance units from any Enemy Model.
...
- **Statline Component**: Commanders may have Characteristic values that differ from standard Units.
...
A Commander may be deployable to the Battlefield as a Unit (a Commander Unit). When deployed, a Commander Unit follows all standard Unit rules. A Commander Unit's Composition Component, Statline Component, and Equipped Items Component are specified in the Commander's definition.

A Commander that is not deployable as a Unit still leads the Battleforce and may provide strategic benefits, but does not appear on the Battlefield.

### 260.2 Commander–Army Relationship

Each Commander Entity leads exactly one Army Entity. A Player may have multiple Commander Entities, each with their own Army. A Unit belongs to exactly one Army at a time.

When composing a Battleforce for a Battle (see rule 110.2 in Battle Rules), the owning Player selects Units from the Commander's Army. A Battleforce must be led by at least one Commander at the start of a Battle.

### Examples

#### Example 1: Generic Commander
**Setup**: A generic Commander Entity with a Statline Component slightly above a standard infantry Model (e.g., Endurance 12, Durability 15, Morale 14, Speed 10, Reflex 12), equipped with standard gear via its Equipped Items Component and no special Ability Components.
**Meaning**: The Commander is deployable as a Commander Unit and participates directly in Battle.
...
An Army is an Entity of the Army archetype — its Components include a Commander Reference Component, a Unit References Component, and the `Army` marker Component. An Army is the complete roster of Units available to a Commander. An Army Entity has:

- **Commander Reference Component**: A reference to the Commander Entity leading the Army (see rule 260.1).
- **Unit References Component**: A list of Unit Entity references representing all Units the Commander has rallied.
- **Marker Components**: An Army inherits all marker Components (Tags) from all Units in the Army.

An Army has no innate limits on size or composition. Limits are imposed by the Campaign, Roster, or scenario rules.

### 270.2 Battleforce

A Battleforce is an Entity of the Battleforce archetype — a subset of an Army's Units selected for a specific Battle. A Battleforce Entity has the `Battleforce` marker Component and the following Components:

- **Army Reference Component**: The source Army.
- **Unit References Component**: A subset of the Army's Unit references.
- **Total Value Component**: Computed as the sum of all Unit Value Components in the Battleforce.

A Battleforce is composed during the Planning Stage (see rule 110.2 in Battle Rules) by selecting Units from the Commander's Army.

- A Unit may belong to at most one Battleforce at a time.
- A Battleforce must satisfy the Roster requirements of the Battle or Campaign.
- All Units in a Battleforce begin with their Zone Component set to Reserves.
...
- Minimum or maximum counts of specific Unit archetypes
...
**Setup**: Commander A has an Army Entity with 5 Unit Entities totalling 900 Value. The Roster requires Battleforces between 400 and 600 Value.
**Action**: Player selects 3 Units totalling 500 Value for the Battleforce's Unit References Component.
**Result**: The Battleforce is valid. The remaining 2 Units stay in the Army but are not part of this Battle.

---

## Glossary Additions

- **Ability Component**: A Component attached to a Unit Entity providing capabilities beyond the Statline and equipment. Classified as Passive, Active, or Surge. (Rule 250.1)
...
- **Army**: An Entity of the Army archetype grouping all Unit Entities available to a Commander. Inherits marker Components from its Units. (Rule 270.1)
...
- **Baseline Competence**: A derived value representing a Model's general attack effectiveness, computed by the Combat System from the Statline Component's Endurance. (Rule 210.7a)
- **Battleforce**: An Entity of the Battleforce archetype — a subset of an Army's Units selected for a specific Battle; must satisfy Roster requirements. (Rule 270.2)
- **Bearer**: A Model Entity that has an Item equipped in its Equipped Items Component. (Rule 230.2)
- **Broken**: The worst Morale state in the Morale State Component. Units with Broken Morale cannot Charge or use Surge Actions. (Rule 210.4)
...
- **Commander Unit**: A Commander Entity that is deployable to the Battlefield as a Unit, following all standard Unit rules. (Rule 260.1)
...
- **Composition Component**: A Component on a Unit Entity defining the Models and their counts that make up the Unit. (Rule 240.2)
...
- **Datasheet**: A comprehensive Unit reference compiled from the Unit's Component data, showing Composition, Statline, equipment, Abilities, and Value. (Rule 240.5)
...
- **Destruction State Component**: A boolean Component on a Model Entity indicating whether the Model has been destroyed by the Combat System. (Rule 230.1)
- **Durability**: A Characteristic in the Statline Component representing how much damage a Model can absorb before the Combat System marks it as destroyed. (Rule 210.3)
...
- **Endurance**: A Characteristic in the Statline Component representing a Model's capacity for sustained exertion. (Rule 210.2)
...
- **Item**: An Entity of the Item archetype providing additional rules through its Components when equipped to a Model. (Rule 220.1)
- **Item Option**: An armybuilding option allowing equipment changes on Models within a Unit. (Rule 240.2)
...
- **Model**: An Entity of the Model archetype representing a single individual within a Unit, carrying Statline, equipment, and destruction state Components. (Rule 230.1)
- **Model Keywords**: Marker Components capturing granular Model-level traits. (Rule 200.3)
- **Model Option**: An armybuilding option allowing additional or replacement Models in a Unit. (Rule 240.2)
...
- **Morale**: A Characteristic in the Statline Component (integer) and a Morale State Component (Broken, Poor, Neutral, Good, Surging) governing a Unit's willingness to perform certain actions. Managed by the Morale System. (Rule 210.4)
- **Morale Baseline**: The Morale state value in the Morale State Component that the Morale System trends the Unit toward over time; default is Neutral. (Rule 210.4)
- **Morale State Component**: A Component on a Unit Entity holding the current Morale state and Morale Baseline, managed by the Morale System. (Rule 210.4)
- **Neutral**: The default Morale state in the Morale State Component. (Rule 210.4)
- **Passive Ability Component**: An Ability Component always in effect while the Unit is in play and meets stated conditions. Applied continuously by the Ability System. (Rule 250.2)
...
- **System**: A unit of logic operating on Entities with specific Components. Systems read Component data, perform calculations, and write results back. (Rule 200.4)
- **Unit**: An Entity of the Unit archetype grouping Model Entities, providing the collective interface through which Models engage with Zones and other Entities. (Rule 240.1)
- **Unit Keywords**: Marker Components describing a Unit's battlefield role. (Rule 200.3)
- **Value Component**: An integer Component on a Unit Entity representing overall strength for armybuilding, Roster validation, and scoring. (Rule 240.6)
...
**Statline Component (Model archetype)**:
...
- `UnitKeywordsComponent`: `set<string>` — Unit role marker Components
- `CompositionComponent`: `list<{ model_id: string, count: int }>`
- `ModelInstancesComponent`: `list<ModelEntityRef>` — one per actual Model in the Unit
...
- `UnitRefsComponent`: `{ unit_ids: list<string> }`

**Battleforce-specific Components**:
- `ArmyRefComponent`: `{ army_id: string }`
- `UnitRefsComponent`: `{ unit_ids: list<string> }` — subset of Army's units
...
- Model Options and Item Options are resolved during armybuilding; the engine stores the final Composition Component and Equipped Items Components, not the option selections. The option history may be stored separately for validation replay.
- The Composition Component (list of Model references with counts) expands into individual Model Entity instances at runtime. Each instance tracks its own DamageTakenComponent, DestructionStateComponent, and EquippedItemsComponent independently.
- Armour stacking: when a Model has multiple Items with Armour Components, the Combat System sums Armour Values and sums Resistances per Damage Type, then applies Effective Armour in a single step during the Defense Pipeline.
...
|---|---|---|
| Unit archetype (rule 240.1) | `Units.json` | id, display_name, faction_keywords, unit_keywords, characteristics, value, composition, abilities, model_options, item_options |
| Model archetype (rule 230.1) | `Models.json` | id, display_name, items |
...
- **Battle Rules** (100-series): Uses Commander, Battleforce, and Unit archetypes; Statline, Zone, and Morale State Components.
...
- **Movement and Positioning** (300-series): Uses Speed from Statline Component; Unit archetype; Zone Component. Processed by the Movement System.
...
- **Armybuilding** (within Campaign rules): Uses Value Component, Composition Component, Model Options, Item Options, Rosters. Validated by the Zone System.
...
|---|---|---|
| 0.1 | 2026-03-17 | Initial draft: Entities, Tags, Characteristics, Items, Attack Profiles, Models, Units, Abilities, Commanders, Armies, Battleforces |
...
| 0.4 | 2026-03-18 | Added Appendix B: Tabletop Holdover Review — identified 8 tabletop-derived concepts (Datasheets, Bearer relationship, Tag Inheritance, State Inheritance, Armour Type label, Equipment constraints as prose, Roster-as-Zone, Model-Unit resolution hybrid) with digital-native alternative proposals |
...
|---|---|---|---|---|
| StatlineComponent | Data | Model, Commander | Endurance, Durability, Morale, Speed, Reflex (integers ≥ 0) | 210.1 |
| DamageTakenComponent | Data | Model | Integer: sub-lethal damage accumulated | 230.1 |
| DestructionStateComponent | Data | Model | Boolean: whether the Model is destroyed | 230.1 |
...
| Component | Category | Attached To | Description | Defining Rule |
|---|---|---|---|---|
| EquippedItemsComponent | Data | Model, Commander | List of Item references with counts | 230.1 |
...
| Component | Category | Attached To | Description | Defining Rule |
|---|---|---|---|---|
| FactionKeywordsComponent | Marker | Unit | Faction allegiance keywords | 240.1 |
| UnitKeywordsComponent | Marker | Unit | Battlefield role keywords | 240.1 |
| CompositionComponent | Data | Unit | Model references with counts | 240.2 |
| ModelInstancesComponent | Data | Unit | Runtime list of Model Entity references | 240.2 |
| ValueComponent | Data | Unit | Integer: armybuilding and scoring strength | 240.6 |
| AbilityRefsComponent | Data | Unit | List of Ability references | 250.1 |
| MoraleStateComponent | Data | Unit | Current Morale state + Morale Baseline | 210.4 |
| EnduranceRemainingComponent | Data | Unit | Integer: current Endurance pool | 240.1 |
| ZoneComponent | Data | Unit, Model | Current Zone (Battlefield, Reserves, Embarked, Casualty Report, Confirmed KIA) | 200.4 |
| PositionComponent | Data | Unit, Model | Grid coordinates on the Battlefield | Movement Rules (300-series) |
| TemporaryTagsComponent | Data | Unit | Tags with expiry metadata (Status Tags with duration) | — |
...
| UnitRefsComponent | Data | Army, Battleforce | List of Unit Entity references | 270.1, 270.2 |
| TotalValueComponent | Data | Battleforce | Computed sum of Unit Values | 270.2 |
| IsDeployableComponent | Data | Commander | Whether the Commander deploys as a Unit on the Battlefield | 260.1 |
...
| Faction Keyword | Marker Component (set) | Permanent per Unit definition | `Human`, `Xenos`, `Chaos` |
| Unit Keyword | Marker Component (set) | Permanent per Unit definition | `Infantry`, `Vehicle`, `Artillery`, `Recon` |
| Model Keyword | Marker Component (set) | Permanent per Model definition | `Heavy Weapon`, `Medic`, `Pilot` |
...
|---|---|---|---|
| **Model** | StatlineComponent, EquippedItemsComponent, DamageTakenComponent, DestructionStateComponent | — | 230.1 |
| **Unit** | StatlineComponent, FactionKeywordsComponent, UnitKeywordsComponent, CompositionComponent, ValueComponent, ZoneComponent, MoraleStateComponent, EnduranceRemainingComponent | — | 240.1 |
...
| **Army** | CommanderRefComponent, UnitRefsComponent | `Army` | 270.1 |
| **Battleforce** | ArmyRefComponent, UnitRefsComponent, TotalValueComponent | `Battleforce` | 270.2 |
...
**Holdover**: A Datasheet is defined as "a comprehensive reference document for a Unit." This is a printed reference card concept from tabletop wargaming — a physical artifact Players hold in hand during play.
...
**Proposed alternative**: Reclassify Datasheets as a **UI specification** rather than a game rule. Remove rule 240.5 from Entity System Rules and relocate the field list to a UI/presentation document. Any rule that references "a Model's Datasheet" should reference the Model's Components directly. The Terminology entry for Datasheets should describe them as a convenience view, not a game object.

### B.2 Bearer Relationship (Rule 230.2)

**Holdover**: The "Bearer" relationship is a named concept from tabletop wargaming ("the bearer of this weapon gains..."). Rule 230.2 defines four properties of this relationship, but in ECS terms, "bearing" an Item is simply having a reference to it in the Model's EquippedItemsComponent.
...
- Point 4 (Items on destruction): Define as a Zone System behavior: "When the Zone System transitions a Model to Casualty Report, the Model's EquippedItemsComponent is preserved for reporting but Items are no longer accessible to other Systems."
...
**Holdover**: "A Unit inherits all marker Components from all Models in the Unit." In tabletop, you cannot efficiently query every Model in a Unit; you need the Unit itself to "have" the tag so you can check once. This is a physical-play optimization.

**Problem in digital**: Automatic tag propagation creates implicit state changes. When the Combat System destroys a Model, the tag inheritance rule requires recalculating the Unit's effective marker Component set — a side effect that crosses System boundaries. It also creates unexpected behavior: a single special Model (e.g., a Medic with the `Medic` marker Component) makes the entire Unit "have" the `Medic` tag, which may cause rules to apply at the Unit level that were intended for the Model level.

**Proposed alternative**: Replace tag inheritance with **System-level queries**. Instead of "Units with the Infantry tag," rules should target "Units containing at least one Model with the `Infantry` marker Component." Systems perform this query against Model Components directly. This:
- Eliminates implicit state propagation
- Makes rule targeting explicit (does the rule care about "any Model" or "all Models"?)
- Removes the recalculation requirement on Model destruction
- Allows rules to distinguish "Unit where any Model has tag X" from "Unit where all Models have tag X"

If Unit-level tags are needed for performance in the digital implementation, they can be maintained as a **computed cache** (like derived values in rule 210.7) rather than a rules-level inheritance mechanism.

### B.4 State Inheritance (Rule 240.4)

**Holdover**: "A Unit can inherit states from its Models when all Models share that state." The primary example is destruction: a Unit is destroyed only when all its Models are destroyed. This is a tabletop convention where you remove the last model from the base and declare the unit destroyed.
...
- **Destruction**: "The Zone System transitions a Unit's Zone Component to Casualty Report when `all(model.DestructionStateComponent.is_destroyed for model in unit.models)` evaluates to true." This is a Zone System behavior, not an Entity-level inheritance rule.
...
**Holdover**: The Armour Component requires an `armour_type: string` described as a "Descriptive label for the armour category (e.g., composite, heavy_plate, energy_shield)." This is a tabletop convention where armour categories are listed for flavor and loose rule interactions ("affects models wearing power armour").
...
**Holdover**: Equipment limitations are described in informal prose: "Equipment categories (e.g., 'a Model may equip at most one Item with a heavy_plate Armour Component') are defined by the Unit's Composition rules or by Item-specific restrictions." This mirrors tabletop datasheets where equipment limits are stated in natural language footnotes.

**Problem in digital**: Prose-defined constraints are not machine-readable. A System cannot validate equipment legality without a human-authored parser for each constraint variant. The constraints are scattered across Unit Composition rules and Item definitions with no standard format.

**Proposed alternative**: Formalize equipment constraints as **Components or structured data**:
- An `EquipmentConstraintsComponent` on Unit or Model Entities defining slot limits, mutual exclusions, and type restrictions in a structured format (e.g., `{ max_armour: 1, max_heavy_weapon: 1, excluded_combinations: [...] }`).
- Alternatively, an `EquipmentSlotComponent` system where each Model has named slots with type constraints, and Items declare which slots they occupy.
...
### B.8 Model-Unit Resolution Hybrid

**Holdover**: Orders are issued to Units (rule 132.1 in Battle Rules), but the Combat System resolves attacks Model-by-Model (rule 133.5). Position is tracked at the Unit level (PositionComponent on Unit), with Models having RelativePositionComponents within the Unit's formation. This hybrid exists because tabletop games move unit bases as a group but track individual model casualties with removable miniatures.

**Problem in digital**: The split creates complexity in the ECS architecture. The Combat System must distribute Unit-level attack Orders to Model-level targets, resolve per-Model, then aggregate results back to the Unit level for Morale and Zone transitions. Position is managed at two levels (Unit absolute, Model relative). Systems must constantly bridge the Unit/Model boundary.

**Proposed alternative**: This is more a design review than a direct holdover fix. Three options:
- **(a) Keep as-is, but justify**: Document why the two-level resolution exists as a deliberate design choice (per-Model casualties create tactical depth; Unit-level Orders keep cognitive load manageable). Add this rationale to the design document.
- **(b) Fully Unit-level**: Abstract away individual Models. Units have a "strength" or "model count" value that decrements as damage accumulates. Simpler but loses per-Model equipment variety and targeted attacks.
- **(c) Fully Model-level**: Each Model has its own Order. Maximum tactical depth but likely too complex for Players to manage.

Option (a) is recommended, with explicit documentation of the design rationale and clear System responsibility boundaries for the Unit/Model bridge.
