...
**Failing Text**: > "Baseline Competence is derived from the Model's Endurance. The exact derivation is specified in the Combat Resolution rules."
...
**Problem**: "Sub-lethal damage (damage below Durability) is tracked on the Model for future resolution." No rule in any document specifies when or how accumulated sub-lethal damage resolves. Does damage accumulate across turns? If a Model with 10 Durability takes 4 damage in Turn 1 and 6 damage in Turn 2, is it destroyed?
**Failing Text**: > "Sub-lethal damage (damage below Durability) is tracked on the Model for future resolution."
**Recommendation**: Add explicit damage accumulation rule: "Damage instances accumulate on the Model. When total accumulated damage on a Model meets or exceeds that Model's Durability, the Model is destroyed." Or specify that each damage instance is resolved independently (no accumulation). The implementation notes already include `damage_taken: integer`, implying accumulation is intended.
...
**Problem**: Melee is defined in the glossary as "A state in which two or more opposing Units are within 1 distance unit" but it is also described as something Units are "placed in" (132.2e). It is unclear whether Melee is: (a) a tag/status applied to Units when a Charge connects, or (b) a spatial condition re-evaluated each Phase. This distinction matters for: Can a Unit exit Melee by being pushed/repositioned away? Is Melee a zone?
**Failing Text**: > "the charging Unit and the target Unit are both placed in Melee"
**Recommendation**: Define Melee explicitly as a tracked state (tag) applied when a Charge makes contact or when Units begin a Turn within 1 DU. Specify when the Melee tag is removed (e.g., when one Unit Falls Back, when one Unit is destroyed, when separation exceeds 1 DU at the start of a Phase).

---

### E04: Endurance Scope — Model vs Unit
**Issue Type**: Language Precision
**Severity**: ERROR
**Location**: Entity System Rules 210.1–210.2; Battle Rules 132.4, 131.1
**Problem**: Endurance is defined as a Model-level Characteristic (210.1: "Every Model has exactly five Characteristics"), but all Battle Rules operate on Endurance at the Unit level ("Unit's Endurance," 132.4). Implementation notes store `endurance_remaining` per Unit. If Models in a Unit have different Endurance values, the Unit-level Endurance is undefined.
**Failing Text**: > "Endurance is an integer Characteristic representing a Model's capacity..." (210.2) vs. "A Unit with zero remaining Endurance" (132.4)
**Recommendation**: Clarify that Endurance functions at the Unit level for Order restrictions and tracking (using the lowest Model Endurance as the Unit's value, or the average, or first Model's value). State explicitly which resolution applies.
...
**Problem**: Melee Resolution in 134.1 says "Units in Melee that have not yet attacked this Turn exchange attacks." A Unit that issued a Hold Order and attacked during 133.5 (Resolve Attacks) has "already attacked." If that Unit was then Charged into Melee, it cannot defend itself in the Melee Resolution step. The snapshot-calculate-apply model means attacks in 133.5 are calculated before Melee is established by Charge movement, creating a paradox.
**Failing Text**: > "Units in Melee that have not yet attacked this Turn exchange attacks simultaneously using melee weapon profiles."
**Recommendation**: Reword to: "All Units in Melee exchange melee attacks, regardless of whether they attacked during the Execute Phase. Ranged attacks during the Execute Phase (rule 133.5) and melee attacks during the Resolve Combat Phase (rule 134.1) are independent combat events."
...
**Problem**: "Distribute attacks from each attacking Model to the closest Model in the target Unit." When two or more target Models are equidistant from the attacker, the rule provides no tiebreaker. The result is non-deterministic.
**Failing Text**: > "Distribute attacks from each attacking Model to the closest Model in the target Unit."
**Recommendation**: Add tiebreaker: "If multiple target Models are equidistant, distribute attacks to the target Model with the lowest remaining Durability. If still tied, distribute to the target Model with the lowest ID (lexicographic)."
...
**Problem**: "Remove all expired temporary effects and tags" — but no rule defines how a tag is marked as temporary or when it expires. The Dug In tag (132.2f) says it's removed "when the Unit moves or the tag is otherwise removed by a rule," but there's no general mechanism for expiry tracking.
**Failing Text**: > "Remove all expired temporary effects and tags from all Units."
**Recommendation**: Define a Temporary Tag as a tag with an associated expiry condition (e.g., "until end of Turn," "until the Unit moves," "for N Turns"). Add this to Entity System Rules 200.2 or a new sub-rule. The implementation notes already include `temporary_tags: set of strings with expiry metadata` — the rule text should match.
...
**Problem**: "A percentage of destroyed Units (defined by the Campaign) are restored." If 3 Units are destroyed and 50% recovery applies, 1.5 Units would be recovered. No rounding rule is specified. Additionally, it's unclear whether "restored" means the entire Unit at full strength or each Unit with partial Model recovery.
**Failing Text**: > "A percentage of destroyed Units (defined by the Campaign) are restored. The remaining Units are permanently lost."
**Recommendation**: Specify: (1) rounding rule (round down, so 3 × 50% = 1 Unit recovered); (2) whether recovered Units are restored to full Model count or retain their damage state from the Battle. Suggest: "Round down. Recovered Units are restored to full Model count."
...
**Problem**: Reposition Reaction (133.3a) taxes Endurance by 1, but there is no explicit restriction preventing a Unit with 0 Endurance from declaring a Reposition. Rule 132.4 restricts Orders that tax Endurance but does not mention Reactions. Entity System 210.2 says "When a Unit's remaining Endurance reaches 0, that Unit may not receive Orders that tax Endurance" — Reactions are not Orders.
**Failing Text**: > "Cost: Taxes Endurance by 1." (133.3a) with no Endurance requirement stated.
**Recommendation**: Add to 133.3a Requirements: "The reacting Unit must have at least 1 remaining Endurance." Or add a general rule: "Any action that taxes Endurance requires the Unit to have at least 1 remaining Endurance."
...
**Problem**: "When Models are destroyed, the footprint may shrink as the remaining Models consolidate." The word "may" implies this is optional or conditional, but it's unclear who decides and when consolidation occurs. Footprint size directly affects collision, movement, and deployment validity.
**Failing Text**: > "When Models are destroyed, the footprint may shrink as the remaining Models consolidate."
**Recommendation**: Replace with: "When Models are destroyed, the Unit's footprint recalculates based on the remaining Model count and the current formation (rule 320.3). The footprint dimensions update at the start of the Unit's next movement."
...
**Problem**: Euclidean distance uses square roots, producing irrational numbers. Threshold comparisons ("within 3 distance units") require floating-point comparison (e.g., √8 ≈ 2.8284...). Floating-point equality is error-prone.
...
**Problem**: "If a Unit's Morale state is currently Surging, the Unit's Morale resets to Neutral instead." But a Unit's Morale Baseline may be something other than Neutral (e.g., Good, if granted by an Ability). The rule always resets Surging to Neutral, even if the baseline is higher.
**Recommendation**: Confirm this is intentional (Surging always costs you by resetting to Neutral, even if your baseline is Good). If so, add a note. If not, change to "resets to the Unit's Morale Baseline."
...
**Problem**: "Moves directly away from the nearest enemy Unit." If two enemy Units are equidistant, "nearest" is ambiguous. Also, "directly away" could be misread as requiring a straight-line path when the path may need to route around obstacles.
...
**Problem**: Formation defaults are defined per keyword (5 files for Infantry, 3 for Vehicle), but a Unit with both tags has no specified default.
**Recommendation**: Add precedence: "If a Unit has both Infantry and Vehicle keywords, use the Vehicle formation default (3 files)."
...
**Problem**: "The Return Fire attack uses the reacting Unit's ranged weapon profiles with a −1 modifier to hit probability (representing hasty fire)." It's unclear whether "−1 modifier to hit probability" means −1 percentage point to the final probability or −1 to the additive modifier stack.
...
**Problem**: Rule 330.2 says "The pivot cost is subtracted from the Unit's available movement distance." Rule 330.3 says "The cost of this adjustment is included in the path length." These are mathematically equivalent but use opposite framing (subtract from budget vs add to cost). This may confuse implementers.
**Recommendation**: Pick one framing consistently. Suggest: "Pivot cost is subtracted from the Unit's available movement distance before calculating the remaining path length."
...
**Problem**: "Each Unit that did not perform an Endurance-taxing action during the previous Turn." The term "Endurance-taxing action" is used but not formally defined as a category. Currently: Run, Charge, and Reposition Reaction tax Endurance.
...
**Problem**: "The number of charges is defined by the count of the Item in the Model's equipment list." This conflates Item count with charges. If a Model has 3 Frag Grenades, does each grenade have 1 use (3 total uses), or does the "Frag Grenade item" have 3 charges?
**Recommendation**: Clarify: "Each instance of a Consumable in the equipment list represents one charge. A Model equipped with Frag Grenades (count 3) has 3 charges of the Frag Grenade Consumable."
...
- **Simultaneous Resolution model** (snapshot-calculate-apply): Excellent design. Fully deterministic by construction.
...
- **Models** (230.1–230.2): Clear bearer relationship.
- **Units** (240.1–240.6): Tag inheritance and State inheritance well-specified.
...
- **Sector Map** (810.1–810.6): Clean graph model with validation constraints.
...
3. **Clarify Endurance scope** (E04) — Decide Unit vs Model and document the resolution. The PoC implementation needs this for Order validation.
